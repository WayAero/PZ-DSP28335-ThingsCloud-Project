...
普中DSP28335 相关公开教程和例程年代较早，直接组合使用时会遇到工程配置、内存链接、外设引脚冲突、阻塞式外设调用等问题。本仓库保留了一份已联调通过的工程状态，并把调试过程中遇到的问题整理在 README 和文档中，方便后续学习者少走弯路。
...
| 项目 | 版本 / 配置 |
| --- | --- |
| IDE | Code Composer Studio 7.x |
| CCS 本地目录 | `ccsv7` |
| C2000 编译器 | `ti-cgt-c2000_16.9.6.LTS` |
| 目标芯片 | TMS320F28335 |
| 调试器配置 | `targetConfigs/TMS320F28335.ccxml` |
| 链接脚本 | `DSP2833x_Libraries/28335_RAM_lnk.cmd` |
| 串口参数 | `9600 8N1` |

项目使用 RAM 运行配置，适合课程设计、调试和演示。若要改为 Flash 固化运行，需要重新调整启动文件、链接脚本和 Flash 初始化流程。

配套 ESP32 工程：

```text
https://github.com/WayAero/ESP32-ThingsCloud-Bridge
```

配套云平台：

```text
ThingsCloud 设备属性上报 / 属性下发
```

## Codex 辅助说明

本项目开发过程中使用 Codex 辅助完成代码审查、问题定位、README 整理、Git 分支清理和公开仓库隐私检查。

硬件现象、串口通信、ThingsCloud 联调结果均以实际测试为准。Codex 只作为辅助工具，不能替代硬件接线、电压、电平和外设负载能力检查。
...
建议按这个顺序复现，不要一开始就做云端闭环：

1. 先单独下载 DSP 工程，确认串口每秒输出 JSON。
2. 用串口助手直接给 DSP 发送 JSON 命令，确认 LED、继电器、蜂鸣器、电机能动作。
3. 调节 ADC，确认 LCD9648 电压显示和报警逻辑。
4. 再接 ESP32 的 `GPIO16/GPIO17`，确认 ESP32 日志能看到 `DSP -> ESP32`。
5. 最后接入 ThingsCloud，测试属性上报和属性下发。

如果现象异常，优先检查：

- DSP 是否正在运行最新 `.out`。
- DSP 串口命令是否带 `\n`。
- ESP32 是否接到了 `GPIO16/GPIO17`，而不是 RX0/TX0。
- DSP 和 ESP32 是否共地。
- LCD9648 驱动是否只配置自身引脚，没有整口覆盖 GPIO0-GPIO15。
- 蜂鸣器是否使用 ePWM4A，而不是主循环翻转 GPIO。
